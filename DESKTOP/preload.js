...
try {
  ...
  for (const arg of args) {
    ...
    if (lowerArg.startsWith('--supabaseurl=')) {
      ...
      console.log("✅ Found URL via --supabaseurl (lowercase)");
    }
    else if (lowerArg.startsWith('--supabaseanonkey=')) {
      supabaseAnonKey = arg.split('=')[1];
      console.log("✅ Found Key via --supabaseanonkey (lowercase)");
    }
    else if (lowerArg.startsWith('--supabase-url=')) {
      supabaseUrl = arg.split('=')[1];
      console.log("✅ Found URL via --supabase-url");
    }
    else if (lowerArg.startsWith('--supabase-anon-key=')) {
      supabaseAnonKey = arg.split('=')[1];
      console.log("✅ Found Key via --supabase-anon-key");
    }
    else if (lowerArg.startsWith('--supabaseurl=')) {
      supabaseUrl = arg.split('=')[1];
      console.log("✅ Found URL via --supabaseUrl");
    }
    else if (lowerArg.startsWith('--supabaseanonkey=')) {
      supabaseAnonKey = arg.split('=')[1];
      console.log("✅ Found Key via --supabaseAnonKey");
    }
  }
